...
def plot_stations_on_layout(ax, layout):
    """
    Plot the positions of stations on the layout.
    
    Parameters:
    - ax: Matplotlib axis object where the stations will be plotted.
    - layout: An array of shape (N, 4) where each row represents a station with format [x, y, w, h].
    """
    for station in layout:
        x, y, w, h = station
        ax.plot(x, y, 'ko', markersize=5)  # 'ko' -> black circle
        # ax.text(x, y, f'({x:.1f}, {y:.1f})', fontsize=9, ha='right')  # Optional: label coordinates

def plot_clusters_on_layout(ax, layout, cluster_assignments):
    """
    Visualize the clustering results on the layout.
    
    Parameters:
    - ax: Matplotlib axis object where the clusters will be plotted.
    - layout: An array of shape (N, 4), similar to plot_stations_on_layout.
    - cluster_assignments: An array of shape (N,) with cluster labels for each station.
    """
    import numpy as np
    colors = plt.cm.get_cmap('viridis', np.max(cluster_assignments) + 1)  # Colormap for clusters

    for i, (x, y, _, _) in enumerate(layout):
        cluster_id = cluster_assignments[i]
        ax.plot(x, y, 'o', markersize=10, color=colors(cluster_id))  # Plot with color based on cluster id
        ax.text(x, y, str(cluster_id), color='white', fontsize=12, ha='center', va='center')  # Label each point with its cluster id
